...
Generated from `git log` (full history, 2026-02-08 → 2026-07-20). Each record groups related commits around one architectural decision. Status reflects the state as of the latest commit (`1e98095`), not necessarily the original intent.
...
**Note (uncommitted at time of writing)**: A further migration moved `docs/analyzed/*.md` → `.claude/analyzed/*.md` (this file's own directory), with `docs/analyzed/` deleted and `.claude/CLAUDE.md`'s workflow references repointed accordingly. This change is not yet reflected in `git log` as of commit `1e98095` — see [[notes]] for details once committed.
...
| 010 | Docs moved to `.claude/` + `docs/analyzed/` | Accepted |
